export default function Error({ message, error, onRetry }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[300px] sm:min-h-[400px] text-center px-4">
      ...
      <h3 className="text-xl sm:text-2xl font-semibold mb-2 text-slate-900 dark:text-white">{errorDisplay.title}</h3>
      <p className="text-slate-600 dark:text-gray-400 max-w-md mb-4 text-sm sm:text-base">{errorMessage}</p>
      
      {errorDisplay.suggestions && errorDisplay.suggestions.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 max-w-md mb-4 border border-slate-200 dark:border-gray-700 shadow-md shadow-slate-200/50 dark:shadow-black/20">
          <p className="text-sm font-semibold mb-2 text-slate-900 dark:text-white">Suggestions:</p>
          <ul className="text-sm text-slate-600 dark:text-gray-400 text-left list-disc list-inside">
